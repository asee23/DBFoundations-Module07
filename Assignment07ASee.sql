...
-- 2017-01-01,ASee,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_ASee')
	 Begin 
	  Alter Database [Assignment07DB_ASee] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_ASee;
	 End
	Create Database Assignment07DB_ASee;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_ASee;
...
SELECT 
	ProductName, UnitPrice = FORMAT(UnitPrice, 'c', 'en-us')
FROM vProducts
...
SELECT 
	CategoryName, ProductName, UnitPrice = FORMAT(UnitPrice, 'c', 'en-us')
...
SELECT 
	...
	InventoryCount = Count
FROM vProducts JOIN vInventories ON vProducts.productID = vInventories.productID
ORDER BY ProductName, CAST([InventoryDate] AS DATE), 3;
GO
...
CREATE FUNCTION fProductInventoriesWithPreviousMonthCountsWithKPIs (@KPIValue int)
    ...
    InventoryCount, 
    PreviousMonthCount,
    CountvsPreviousCount
    FROM vProductInventoriesWithPreviousMonthCountsWithKPIs
    WHERE [CountvsPreviousCount] = @KPIValue;
GO
